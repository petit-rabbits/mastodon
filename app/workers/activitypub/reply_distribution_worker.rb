...
class ActivityPub::ReplyDistributionWorker
  ...
  def payload
    @payload ||= Oj.dump(@status.distributable? ? serialize_and_sign_payload(@status, ActivityPub::ActivitySerializer, @status.account) : serialize_payload(@status, ActivityPub::ActivitySerializer))
  end
end
