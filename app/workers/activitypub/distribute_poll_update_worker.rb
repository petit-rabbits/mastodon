...
class ActivityPub::DistributePollUpdateWorker
  ...
  def payload
    @payload ||= Oj.dump(@status.distributable? ? serialize_and_sign_payload(@status, ActivityPub::UpdatePollSerializer, @account) : serialize_payload(@status, ActivityPub::UpdatePollSerializer))
  end
